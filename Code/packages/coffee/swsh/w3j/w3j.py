...
class W3jStone(object):
    ...
    rrfcalc. For some reason wrapping his fortran code directly with f2py 
    ...
    def __call__(self, j1, j2, j3, m1, m2, m3):
        ...
        Return:
        Returns a the value of the 3j symbol,
        (j1, j2, j3)
        (m1, m2, m3)"""
    ...
    def exact(self, j1, j2, j3, m1, m2, m3):
        ...
        Returns a the value of the 3j symbol,
        (j1, j2, j3)
        (m1, m2, m3) as a string. The format of the string is 
...
class W3jBen(object):
    ...
    Please note, however, that there maybe some degradation in performace as a
    ...
    calcualated. All calculated symbols are stored in the hdf file.
    ...
    def __init__(self, hdf_file):
        ...
        Arguments:
        hdf_file - The name of the hdf_file which stores, or will be used to 
    ...
    def _write_w3j_symbols(self,
        w3j_symbols, j1_min, j1_num, j1_max, j2, j3, m1, m2, m3
        ):
        """Writes an array of 3j symbol values to files, skipping those values
        that already exist."""
        ms = np.array([m1, m2, m3], dtype=typeDict['double'])
...
class W3jBoris(object):
    """This class provides a wrapper to the portions of Boris' code.
    ...
    of the 3j symbol ( j1, j2, j3 )
                     ( m1, m2, m3 ) from object, w, one uses the code 
    w(j1, j2, j3, m1, m2, m3).
    
    The name of the file is currently hard coded into Boris' code and cannot be
    changed.
    
    Calculated values can be writen to the file data.h5 by using the method
    save().
    
    If at some later data is is necessary to increase the bandwidth limit, this
    object requires recalculting all w3j values and storing this information
    ...
    def __init__(self, L, dl):
        ...
        Arguments:
        L - The bandwidth limitor. j2, j3, m1 and m2 are all restricted to be
        ...
        dl - If only integer values of j1, j2, j3, m1, m2 and m3 are required 
    ...
    def save(self):
        ...
        if self._cplCw3jStruct_ptr is None:
            raise Exception("The values cannot be saved as they have not been\
            calculated")
    ...
    def calculate(self):
        ...
        print "Performing calculation of w3j symbols"
        self._cplCw3jStruct_ptr = self._cg.calcw3jSymbols(self.L, self.dl)
        print "Calculation of w3j symbols complete"
